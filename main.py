"""
Zerodha Market Connect Pro (MCP) - Main Application Module

This module serves as the main entry point for the Zerodha MCP trading system.
It provides a FastAPI-based server that handles:
1. Authentication with Zerodha's Kite Connect API
2. Session management and token persistence
3. Trading operations (orders, positions, holdings)
4. Market data access
5. Mutual fund operations

The application uses FastMCP for tool management and provides a comprehensive
set of trading and analysis tools that can be used programmatically.
...
app = FastAPI(
    title="Zerodha MCP Trading System",
    description="A sophisticated trading automation system for Zerodha",
    version="0.1.0"
)
...
class ZerodhaContext:
    ...
    Attributes:
        kite: KiteConnect instance for API interactions
        api_key: Zerodha API key
        api_secret: Zerodha API secret
        app: FastAPI application instance
        server_thread: Optional thread running the FastAPI server
    """
    kite: KiteConnect
    api_key: str
    api_secret: str
...
def load_stored_token() -> Optional[str]:
    """
    Load previously stored access token from file system.
    
    Returns:
        str: The stored access token if available, None otherwise
    """
    try:
...
def save_access_token(token: str):
    """
    Persist access token to file system for future sessions.
...
def start_server():
    """
    Start the FastAPI server for handling Zerodha callbacks.
    Runs on localhost:5000 with minimal logging.
    """
